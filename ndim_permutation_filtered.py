...
def ndimensional_permute(permlist: list, dimensions: int, max_duplicates: 
    ...
    # -2 because dimensions dictates permutation while loop
    # and initialized permuted_list is already 2D
    dim = 2
    permuted_list = [[[y, x] for x in permlist] for y in permlist]  # init with 2D permutation
    ...
    while loop_condition > 0:
        ...
        dim += 1
        if verbose > 0:
            print(f'ndim_permute says:\tdimensions = {dim}\t{max_duplicates = }\t{len(flatten_list(permuted_list)) = }\n')

    if dimensions - 2 < 1 and verbose > 0:
        print(
            f'ndim_permute says:\tdimensions = {dim}\t{max_duplicates = }\t{len(flatten_list(permuted_list)) = }\n')

    orig_len = len(flatten_list(permuted_list))

    # filtering out permutations with more than max_duplicates of the same item in a given permutation
    if max_duplicates > 0:
        for sublist in permuted_list:
            ...
            for i in range(len(sublist)):
                ...
                # for each item check if permutation contains more than max_duplicates of the same item
                for item in perm:
            ...
            counter = 0
            # only after all permutations from sublist have been filtered, add sublist to filtered_permuted_list
    ...
    if verbose > 0:
        print(f'filtered {leftovers} permutations of {orig_len} containing more than {max_duplicates} duplicates\n')
...
def ndimensional_filter(data_list: list, dimensional_filterlist: list[list], returnwhich: str = 'filtered', filtermode: str = 'loose',
    ...
    dimensional_filterlist: n-dimensional list of lists of items to filter out. format is [[dimension 1], [dimension 2], ...].
        if filtermode is loose empty inner lists are needed to declare dimensionality in case of filtering eg only last and first
        dimension of 3 or higher dimensional data: dimensional_filterlist = [[1,2], [], [3]]
    returnwhich: takes 'filtered' or 'unfiltered' as a string. if 'filtered', only filtered permutations will be returned.
        otherwise filtered permutations will be deleted from permutation list and leftovers will be returned.
    filtermode: takes 'strict' or 'loose' as a string. if 'strict', permutations which strictly contain only items which are set
        in dimensional_filterlist will be filtered. if 'loose', permutations which contain at least one item which is set in 
        dimensional_filterlist in a given dimension are filtered.
    flattend: if True, returns a flattened list of permuted entries.
    verbose: takes values from 0-2 inclusively. 0 = silent, 1 = print filtered list, 2 = print all
    print_param: int to dictate how many lines of printout should be generated from within loops. use 0 for infinite.
    '''
    if returnwhich not in ['filtered', 'unfiltered']:
        returnwhich = 'filtered'
        print('returnwhich must be either "filtered" or "unfiltered". defaulting to "filtered"')
    if filtermode not in ['loose', 'strict']:
        filtermode = 'loose'
        print('returnwhich must be either "loose" or "strict". defaulting to "loose"')

    permutations_taken = len(flatten_list(data_list))
    if verbose > 0:
        print(f'ndim_filter says:\toverall number of permutations = {permutations_taken}\t{dimensional_filterlist = }\t{returnwhich = }\t{filtermode = }\tverbose level = {verbose}\t{print_param = }\n')
    filtered_list = []
    unfiltered_list = []
    first_init = True
                ...
                for filter_index, filter in enumerate(filter_values):
                    ...
                    for data_sub_list in data_list: 
                        ...
                        for permutation_index, perm in enumerate(data_sub_list):
                            # if filter values DOES match perm[dimension] append permutation to filtered_sub_list
                            ...
                            # if filter values DOES NOT match perm[dimension] append permutation to unfiltered_sub_list
                    ...
                    if verbose > 0:
                        print(f'filtered: {len(flatten_list(filtered_list))}\tunfiltered: {len(flatten_list(unfiltered_list))}\t{dimension = }\t{filter_values = }\tactive filter: {filter}\n')
                        
            elif filter_values == [] and not first_init:
                filtered_list = data_list

            if verbose > 0:
                print(f'filtered: {len(flatten_list(filtered_list))}\tunfiltered: {len(flatten_list(unfiltered_list))}\t{dimension = }\t{filter_values = }') 
        ...
        for dimension, filter_values in enumerate(dimensional_filterlist):
            for filter_index, filter in enumerate(filter_values):
                ...
                for data_sub_list in data_list:
                    ...
                    filtered_sub_list = []
                    unfiltered_sub_list = []
                    # permutations to be removed from filtered_list if found to be illegal when cheking later filter values
                    late_illegals = []

                    for permutation_index, perm in enumerate(data_sub_list):
                        permutation_is_legal = False

                        # if filter value DOES NOT match perm[dimension], permutation is illegal. if not yet in any list append permutation to unfiltered_sub_list
                        ...
                        # if filter value DOES match perm[dimension], permutation is legal. if not yet in any filtered_list append to filtered_sub_list
                        elif perm[dimension] == filter and perm not in flatten_list(filtered_list) and perm not in filtered_sub_list:
                            filtered_sub_list.append(perm)
                            if print_param > permutation_index and verbose > 2:
                                print(f'filtered: {perm = }')

                        # if filter value DOES NOT match perm[dimension], permutation is illegal. if not first dimension and perm in filtered_list
                        # check for legal possibilities of other filters in this dimension
                    ...
                    if filtered_sub_list != []:                 # add filtered items sublist to filtered_list
                        filtered_list.append(filtered_sub_list)

                    for perm in late_illegals:                  # try deleting all later illegals from filtered_list
                    ...
                    if unfiltered_sub_list != []:               # add unfiltered items sublist to unfiltered_list
                        unfiltered_list.append(unfiltered_sub_list)
    ...
    for perm in filtered_list:                       # sanity check
        if perm in unfiltered_list and verbose > 0:
            print(f'Error in ndimensional_filter: {perm = } has been filtered and unfiltered.')
...
def ndimensional_permute_filtered(permlist: list, dimensional_filterlist: list[list], returnwhich: str = 'filtered', filtermode: str = 'loose',
    ...
    dimensional_filterlist: n-dimensional list of lists of entries to filter out. format is [[dimension 1], [dimension 2], ...].
        len(filtecolumn_widthparlist) = number of inner lists = number of dimensions dictates dimensionality of permutation. if inner lists
        are empty, no filtering will take place (empty inner lists while in 'strict' mode will let no output throught). otherwise,
        all permutations which contain any item of inner lists at defined dimension will be filtered.
    returnwhich: takes 'filtered' or 'unfiltered' as a string. if 'filtered', only filtered permutations will be returned. Otherwise
        filtered permutations will be deleted from permutation list and leftovers will be returned.
    filtermode: takes 'strict' or 'loose' as a string. if 'strict', permutations which strictly contain only items which are set
        in dimensional_filterlist will be filtered. if 'loose', permutations which contain at least one item which is set in 
        dimensional_filterlist in a given dimension are filtered.
    max_duplicates: if set to a number > 0, permutations which contain more than max_duplicates of the same item in a given permutation
    ...
    # permuting, flattened here needs to be False trn
    permuted_list = ndimensional_permute(permlist=permlist, dimensions=dimensions, max_duplicates=max_duplicates, flattened=False, verbose=verbose)
    ...
    # filtering, flattened here needs to be False #
    permuted_filtered_list = ndimensional_filter(data_list=permuted_list, dimensional_filterlist=dimensional_filterlist, returnwhich=returnwhich,
                                                 filtermode=filtermode, flattened=False, verbose=verbose, print_param=print_param)
...
def main_permute():
    ...
    #permuted_filtered1 = ndimensional_permute_filtered(permlist=perm, dimensional_filterlist=filter_list, returnwhich='filtered', filtermode='loose',
                                                       #max_duplicates=1, flattened=True, verbose=2, print_param=2)
    
    #permuted_filtered2 = ndimensional_filter(data_list=permlist, dimensional_filterlist=filter_list, returnwhich='filtered',
                                                #filtermode='loose', flattened=True, verbose=2, print_param=2)
    #print(f'{permuted_filtered1[:3] = }\n')
    permuted_filtered2 = ndimensional_permute_filtered(permlist=perm, dimensional_filterlist=filter_list,
                                                    returnwhich='filtered', filtermode='strict', max_duplicates=1,
                                                    flattened=True, verbose=2, print_param=3)
